...
public enum TaskDueDate: String {
    ...
    func dueDateForMoveDate(_ moveDate: Date) -> Date {
        ...
        return Calendar.current.date(byAdding: components, to: moveDate, wrappingComponents: true)!
    }
    ...
    public func daysFromDueDate(_ moveDate: Date) -> String {
        ...
        let components = calendar.dateComponents(unitFlags, from: Date.startOfToday(),  to: taskDueDate)
        
        if let daysLeft = components.day {
            if daysLeft >= 0 {
                return NSString(format: LocalizedStrings.daysLeftFormat as NSString, daysLeft) as String
            } else {
                return NSString(format: LocalizedStrings.daysAgoFormat as NSString, abs(daysLeft)) as String
            }
        }
        else {
            return ""
