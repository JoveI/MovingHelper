...
class MovingDateViewController: UIViewController {
  ...
  func twoMonthsFromToday() -> Date {
    ...
    twoMonths.month = 2
    let updateDate = currentCalendar.date(byAdding: twoMonths, to: today, wrappingComponents: true)
    return updateDate!
  }
  ...
  @IBAction func datePickerChanged() {
    ...
    let components = calendar.dateComponents(unitFlags, from: today,  to: updatedDate)
    
    daysLeftLabel.text = NSString(format: LocalizedStrings.daysLeftFormat as NSString, components.day ?? -1) as String
  }
